...
public class theServer extends javax.swing.JFrame {
                ...
                while ((message = reader.readLine()) != null) {
                    ...
                    if (data.length >= 3) {
                        switch (data[2]) {
                            ...
                            case "Upload":
                                receiveFile(data[0],reader);
                                break;
                            default:
                                dataStored.append("File Received.\n");
                        }
    ...
    private void receiveFile(String fileName, BufferedReader reader) {
        try {
            ...
            tellEveryone("File transfer: " + fileName + " is being sent.");
    
            StringBuilder fileContentBuilder = new StringBuilder();
            String fileContent;
            while (!(fileContent = reader.readLine()).equals("END")) {
                //fileContentBuilder.append(fileContent).append("\n");
            }

            String fileContentString = fileContentBuilder.toString();
            // Broadcast the file content to all users
            tellEveryone("File: " + fileName + "\n" + "\nFile END");
        } catch (IOException e) {
            dataStored.append("Error receiving file '" + fileName + "'.\n");
            e.printStackTrace();
        }
    }
